...
├── db-analyze.sh           # Анализ и метрики БД ⭐ NEW
...
#### ⭐ Синхронизировать между серверами (NEW!)

```bash
# Обновить DEV из PROD (самый частый сценарий)
./db-sync.sh sync prod dev

# Обновить PROD из DEV (используйте с осторожностью!)
./db-sync.sh sync dev prod
```

**Что делает:**
1. Создает backup на целевом сервере
2. Экспортирует БД с сервера-источника
3. Импортирует на целевой сервер
4. Автоматически заменяет домены
5. Очищает кэш WordPress на целевом сервере

**Когда использовать:**
- 🔄 Обновить DEV данными из PROD для тестирования
- 🧪 Подготовить тестовое окружение с реальными данными
- 🔧 Синхронизировать окружения перед миграцией

⚠️ **ВНИМАНИЕ**: При синхронизации на PROD требуется ввести `REPLACE DATABASE` для подтверждения!
...
### 📊 Анализ базы данных (`db-analyze.sh`) ⭐ NEW

Анализирует базу данных и создает детальный отчет с метриками и рекомендациями.
...
**Что анализируется:**
- Размер БД и таблиц
- Autoload опции (производительность)
- Transient кеш
- Посты, страницы, ревизии
- Custom Post Types
- Комментарии (включая спам)
- Orphaned данные
- Таксономии
- Автоматические рекомендации по оптимизации

**Отчеты сохраняются в:**
- `wordpress/database/reports/db-analysis-{env}-{timestamp}.txt` (текст)
- `wordpress/database/reports/db-analysis-{env}-{timestamp}.json` (JSON)

📖 **Подробнее:** [DB-ANALYZE-README.md](./DB-ANALYZE-README.md)
...
### 007_remove_pods_framework_remnants.sql

**Описание:** Удаляет остатки Pods Framework после миграции на ACF

**Что делает:**
- Удаляет пустую таблицу `wp_podsrel`
- Удаляет все настройки Pods из `wp_options`
- Проверяет, что таблица пустая перед удалением (safety check)
- Выводит результаты верификации
...
- ✅ Все данные из Pods мигрированы в ACF
- ✅ Плагин Pods деактивирован и удален
- ✅ Проверено, что `wp_podsrel` пустая
