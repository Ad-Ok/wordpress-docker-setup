...
Что происходит:
0. **Автоматическое увеличение patch версии темы**
1. Проверка git статуса
2. Git pull на DEV сервере
3. Проверка миграций (если не --skip-migrations)
4. Очистка кеша
5. Smoke tests
...
### 5. Ручное увеличение версии темы 🆕
...
Скрипт автоматически увеличивает последнюю цифру версии в `style.css`.

**Примечание:** 
- При деплое версия увеличивается автоматически
- Для изменения major/minor версии редактируйте `style.css` вручную
- После изменения версии запустите `npm run build` в теме для сборки assets
