...
phase_3_tests() {
    ...
    if [ "$IS_LOCAL" = true ]; then
        ...
        # Для DEV/PROD используем SSH для проверки файлов
        theme_path="${WP_PATH}/wp-content/themes/maslovka"
        remote_check=true
    else
        test_skip "Тесты файловой системы требуют локального доступа или SSH"
        return
    fi
    ...
    if [ "$remote_check" = true ]; then
        if ssh -p "$SSH_PORT" "${SSH_USER}@${SSH_HOST}" "test -f '$mo_file'" 2>/dev/null; then
    ...
    if [ "$remote_check" = true ]; then
        if ssh -p "$SSH_PORT" "${SSH_USER}@${SSH_HOST}" "test -f '$pot_file'" 2>/dev/null; then
    ...
    if [ "$remote_check" = true ]; then
        if ssh -p "$SSH_PORT" "${SSH_USER}@${SSH_HOST}" "grep -q 'load_theme_textdomain.*maslovka' '${theme_path}/inc/theme/theme-setup.php'" 2>/dev/null; then
    ...
    if [ "$remote_check" = true ]; then
        if ssh -p "$SSH_PORT" "${SSH_USER}@${SSH_HOST}" "grep -q '__(' '${theme_path}/404.php'" 2>/dev/null; then
    ...
    if [ "$remote_check" = true ]; then
        if ssh -p "$SSH_PORT" "${SSH_USER}@${SSH_HOST}" "grep -qE 'esc_html_e.*maslovka|__.*maslovka' '${theme_path}/footer.php'" 2>/dev/null; then
    ...
    if [ "$remote_check" = true ]; then
        if ssh -p "$SSH_PORT" "${SSH_USER}@${SSH_HOST}" "grep -qE 'esc_html_e.*maslovka|__.*maslovka' '${theme_path}/single.php'" 2>/dev/null; then
    ...
    if [ "$remote_check" = true ]; then
        if ssh -p "$SSH_PORT" "${SSH_USER}@${SSH_HOST}" "grep -qE 'esc_html_e.*maslovka|__.*maslovka' '${theme_path}/single-collection.php'" 2>/dev/null; then
    ...
    if [ "$remote_check" = true ]; then
        if ssh -p "$SSH_PORT" "${SSH_USER}@${SSH_HOST}" "grep -qE 'esc_attr_e.*maslovka|__.*maslovka' '${theme_path}/page-artists.php'" 2>/dev/null; then
    ...
    if [ "$remote_check" = true ]; then
        if ssh -p "$SSH_PORT" "${SSH_USER}@${SSH_HOST}" "grep -q 'pll_the_languages' '${theme_path}/components/topmenu.php'" 2>/dev/null; then
    ...
    if [ "$remote_check" = true ]; then
        if ssh -p "$SSH_PORT" "${SSH_USER}@${SSH_HOST}" "test -f '${theme_path}/src/scss/components/_language-switcher.scss'" 2>/dev/null; then
    ...
    if [ "$remote_check" = true ]; then
        if ssh -p "$SSH_PORT" "${SSH_USER}@${SSH_HOST}" "grep -q 'english_version_enabled' '${theme_path}/components/topmenu.php'" 2>/dev/null; then
    ...
    if [ "$remote_check" = true ]; then
        if ssh -p "$SSH_PORT" "${SSH_USER}@${SSH_HOST}" "grep -q 'noindex' '${theme_path}/header.php'" 2>/dev/null; then
