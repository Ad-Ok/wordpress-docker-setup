...
Тестовый набор проверяет полную интеграцию Polylang с кастомными типами постов, ACF полями, таксономиями и SEO оптимизацией. Всего: **~96+ автоматических тестов** + интерактивные проверки.
...
**Результат:** Подтверждение, что окружение готово к тестированию Polylang интеграции.
...
### Фаза 2: SQL миграции (EN меню) (18 тестов)
**Цель:** Проверка миграции структуры навигации + полуавтоматический перевод пунктов меню

**Что проверяется:**

**Блок 1: Базовая структура меню (тесты 2.1-2.10)**
- ✅ EN Header меню создано
- ✅ EN Footer меню создано
- ✅ Пункты меню скопированы (Home, телефон, email)
...
- ✅ Меню отображается на фронтенде `/en/`

**Блок 2: Миграция 025 - полуавтоматический перевод (тесты 2.11-2.18)**
- ✅ Миграция `025_polylang_translate_menu_items.sql` существует
- ✅ PHP скрипт `translate-menu-items.php` существует и исполняемый
- ✅ Подсчёт RU и EN пунктов меню (Header + Footer)
- ✅ Количество пунктов совпадает (RU ↔ EN)
- ✅ Проверка переводов для пунктов типа `post_type`
- ✅ Иерархия меню сохранена (родитель-потомок)

**Процесс создания переводов меню:**

**Вариант 1: Автоматический (через db-migrate)**
```bash
cd www/deployment-scripts/database
./db-migrate.sh apply local
```

- **Миграция 025** (SQL) — анализирует меню и создаёт отчёт:
  - `MISSING_TRANSLATION` — страницы без перевода (нужно создать)
  - `TRANSLATION_EXISTS` — переводы готовы (пункты меню создадутся автоматически)
  - `ALREADY_EXISTS` — пункты меню уже существуют
  - `CUSTOM_LINK` — кастомные ссылки (ручной перевод заголовка)

- **Миграция 026** (PHP) — автоматически создаёт пункты меню для существующих переводов

Если есть `MISSING_TRANSLATION`, миграция покажет список страниц с ссылками на wp-admin.

**Вариант 2: Интерактивный (с браузером)**
```bash
cd www/wordpress/database/migrations
php 026_polylang_translate_menu_items.php --env=local --interactive
```

Для каждой непереведённой страницы:
- 🌐 Браузер откроется на `/wp-admin/post.php?post=XXX&action=edit`
- 👆 Нажмите **"+ EN"** в блоке Languages (справа)
- 📝 В новом окне нажмите **"Опубликовать"**
- ✅ Вернитесь в терминал и нажмите **Enter**
- 🚀 Скрипт автоматически создаст пункт меню
...
Ожидаемый результат:
```
✅ [2.14] Количество пунктов EN Header меню совпадает с RU
✅ [2.16] Количество пунктов EN Footer меню совпадает с RU
✅ [2.17] Все пункты меню типа 'post_type' имеют переводы страниц
✅ [2.18] Иерархия меню сохранена
```
...
# Интерактивный режим (после создания переводов)
cd www/wordpress/database/migrations
php 026_polylang_translate_menu_items.php --env=local --interactive
```
