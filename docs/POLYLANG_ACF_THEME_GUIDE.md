...
8. [Кастомный плагин: maslovka-polylang-acf.php](#8-кастомный-плагин-maslovka-polylang-acfphp)
...
## 8️⃣ Кастомный плагин: maslovka-polylang-acf.php

**Расположение:** `wp-content/mu-plugins/maslovka-polylang-acf.php`  
**Версия:** 2.0.0

### Зачем нужен плагин

Polylang FREE имеет ограничения при работе с ACF полями:
- При создании перевода не все ACF поля копируются автоматически
- Polylang фильтрует ссылки на медиафайлы других языков
- Галереи и миниатюры не переносятся корректно

Наш mu-plugin решает эти проблемы.

### Ключевые функции

#### 1. Автоматическое копирование при создании перевода

При нажатии "+ Add new" в панели Languages, плагин автоматически копирует:

| Тип поля | Поведение |
|----------|-----------|
| **post_content** | Полное копирование HTML-контента |
| **post_excerpt** | Полное копирование |
| **Миниатюра** | Копирование с автосозданием EN перевода медиа |
| **Галереи (ACF)** | Копирование всех изображений с автопереводом |
| **Repeaters** | Полное копирование структуры и данных |
| **Даты, числа** | Копирование значений |
| **Текстовые поля** | Копирование для последующего перевода |
| **Таксономии** | Копирование связей (art_form, period) |

#### 2. Автоматическое создание переводов медиафайлов

```php
// Когда в галерее RU изображение без EN перевода,
// плагин автоматически создает EN attachment
maslovka_get_or_create_media_translation($attachment_id, $lang);
```

**Важно:** Файлы НЕ дублируются на диске! Polylang создает новую запись в БД, указывающую на тот же файл.

#### 3. Защита от перезаписи оригинала

Плагин предотвращает обратную синхронизацию — изменения в EN переводе не затрагивают RU оригинал.

### Техническая реализация

```php
// Хук для копирования контента (после Polylang, приоритет 5001)
add_filter('use_block_editor_for_post', 'maslovka_copy_content_on_new_translation', 5001, 2);

// Хук для трансляции ID изображений в галереях
add_filter('pll_translate_post_meta', 'maslovka_translate_attachment_ids', 10, 5);

// Хук для контроля синхронизации (предотвращает обратный sync)
add_filter('pll_copy_post_metas', 'maslovka_control_meta_sync', 10, 5);

// Fallback для таксономий
add_action('pll_save_post', 'maslovka_copy_taxonomies_on_save', 10, 3);
```

### Как это работает (поток)

```
1. Пользователь кликает "+ Add new" (EN)
   ↓
2. Polylang создает auto-draft EN поста
   ↓
3. Polylang копирует мета-поля (приоритет 5000)
   ↓
4. Наш плагин (приоритет 5001):
   a) Копирует post_content через SQL
   b) Находит все attachment ID в метах
   c) Для каждого: получает или создает EN перевод
   d) Заменяет RU ID на EN ID
   ↓
5. Редактор открывается с полностью заполненным переводом
```

### Тестирование

Тесты Фазы 4 в `polylang-test.sh` проверяют:
...
| Тест | Описание |
|------|----------|
| 4.1 | Проверка существования mu-plugin |
| 4.2 | Проверка фильтра защиты от перезаписи |
| 4.3 | Интерактивный тест всех 7 полей |
| 4.4 | Проверка что оригинал не изменён |

### Когда использовать

- ✅ Создание нового EN перевода — всё копируется автоматически
- ✅ Добавление изображений в галерею RU — при создании EN перевода они перенесутся
- ⚠️ Изменение галереи в EN — нужно вручную (плагин работает только при создании)
- ⚠️ Изменение контента RU после создания EN — не синхронизируется (by design)
...
6. ✅ Убедиться что mu-plugin `maslovka-polylang-acf.php` активен
