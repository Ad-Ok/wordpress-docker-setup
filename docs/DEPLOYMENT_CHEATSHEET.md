...
### Деплой с ручными действиями (например, Polylang)

```bash
# 1. Деплой БЕЗ миграций (для плагинов, требующих ручной настройки)
./deploy-dev.sh --skip-migrations

# 2. Выполните ручные действия:
#    - Активируйте плагин (например, Polylang)
#    - Создайте языки, меню и т.д.

# 3. Деплой С миграциями (применит зависимые миграции)
./deploy-dev.sh
```
