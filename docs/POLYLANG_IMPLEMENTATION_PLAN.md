...
### ⚠️ Правильный порядок перевода контента

> **Критически важно!** Порядок перевода контента влияет на корректность связей между сущностями.

**Проблема:**
При переводе коллекции (collection) Polylang копирует простые ACF поля (text, number, select), но **Post Object поля** (например, `artist_id`) ссылаются на ID русского поста. Если у художника ещё нет английского перевода — поле будет пустым или некорректным.

Аналогично с **таксономиями**: при создании перевода поста термины копируются, но если термины не переведены — связи не будут установлены.

**Правильный порядок:**

```
1️⃣ Таксономии (термины)
   └── art_form, genres, materials, period, styles, techniques, event_types, etc.
   
2️⃣ Независимые CPT (без ссылок на другие CPT)
   └── Artists (художники)
   └── Pages (страницы)
   
3️⃣ Зависимые CPT (ссылаются на Artists/термины)
   └── Collections (коллекции) — поле artist_id → Artist
   └── Vistavki (выставки)
   └── Events (события)
   └── Photo
```
...
```bash
# Шаг 1: Перевести все термины таксономий
# Posts → Categories/Tags → перевести каждый термин

# Шаг 2: Перевести всех художников
# Artists → для каждого: Languages → + English → перевести

# Шаг 3: Перевести коллекции
# Collections → Languages → + English
# ✅ artist_id dropdown покажет EN версию художника
# ✅ Таксономии автоматически свяжутся с EN терминами
```

**Что копируется автоматически (Polylang FREE):**

| Тип поля | Копируется? | Примечание |
|----------|-------------|------------|
| Featured image | ✅ Да | `_thumbnail_id` транслируется автоматически |
| ACF Text/Number | ✅ Да | `year_created`, `current_location`, `first_name` |
| ACF Select | ✅ Да | `status` |
| ACF True/False | ✅ Да | Булевы значения |
| ACF Color Picker | ✅ Да | Цвета |
| ACF Repeater | ✅ Да | Весь репитер копируется |
| Taxonomies | ✅ Да | Автоматически связывает с EN терминами (если переведены) |
| ACF Date Picker | ❌ Нет | Не копируется, требует ручного ввода |
| ACF Gallery | ❌ Нет | Не копируется, ID вложений не транслируются |
| ACF Post Object | ❌ Нет | Не копируется, ID поста не транслируется |
| ACF Relationship | ❌ Нет | Не копируется, ID постов не транслируются |
| post_content | ❌ Нет | Контент нужно скопировать вручную |
| post_excerpt | ❌ Нет | Отрывок нужно скопировать вручную |

> **Результаты тестирования (декабрь 2025):**
> - Таксономии работают отлично — EN термины автоматически привязываются
> - Текстовые поля копируются корректно
> - Repeater ("Информационный блок") копируется полностью
> - Gallery/Post Object/Date Picker НЕ копируются — требуют ручного заполнения

**Почему Gallery и Post Object не работают:**

Polylang использует функцию `is_protected_meta()` для определения копируемых полей. Публичные мета-ключи копируются, но **значения НЕ транслируются**:
- Gallery хранит `[123, 456, 789]` — массив ID вложений
- Post Object хранит `123` — ID поста
- При копировании эти ID остаются "русскими", а Polylang не ищет их EN переводы

**Workaround — ручное заполнение:**

Для полей Gallery, Post Object, Date Picker:
1. Создать перевод поста
2. Вручную заполнить эти поля в EN версии
3. Для Gallery — выбрать те же изображения (они общие для всех языков если не переведены)
4. Для Post Object — выбрать EN версию связанного поста (если переведён)

**Опционально — фильтр для автоматической трансляции:**

```php
// Добавить в functions.php или mu-plugin
add_filter('pll_translate_post_meta', 'maslovka_translate_acf_relations', 10, 5);
function maslovka_translate_acf_relations($value, $key, $lang, $from, $to) {
    // Post Object поля — транслировать ID на перевод
    $post_object_fields = ['artist_id', 'автор'];
    if (in_array($key, $post_object_fields) && is_numeric($value)) {
        $translated = pll_get_post($value, $lang);
        return $translated ? $translated : $value;
    }
    
    // Gallery поля — транслировать массив ID
    $gallery_fields = ['работы_художника', 'фото_художника'];
    if (in_array($key, $gallery_fields) && is_array($value)) {
        return array_map(function($id) use ($lang) {
...
> ⚠️ **Примечание:** Этот фильтр требует, чтобы связанные посты/медиа уже были переведены. Если перевода нет — останется оригинальный ID.
