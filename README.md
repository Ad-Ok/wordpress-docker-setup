...
1. **Initial Deploy** (первый раз на сервере)
   - Загружает полный WordPress через rsync
   - Инициализирует Git на сервере
   - Команды: `./initial-deploy.sh dev` или `./initial-deploy.sh prod`

2. **Regular Deploy** (обновления)
   - Инкрементальные обновления через Git
   - Быстрый деплой с проверками
   - Команды: `./deploy-dev.sh` или `./deploy-prod.sh`
...
# Быстрый хотфикс
./deployment-scripts/hotfix.sh
```

**📚 Подробная документация:**
- [DEPLOYMENT_GUIDE.md](docs/DEPLOYMENT_GUIDE.md) — полное руководство
- [DEPLOYMENT_CHEATSHEET.md](docs/DEPLOYMENT_CHEATSHEET.md) — шпаргалка
- [INITIAL_DEPLOY_GUIDE.md](docs/INITIAL_DEPLOY_GUIDE.md) — первоначальный деплой
...
- [DEV_SECURITY.md](docs/DEV_SECURITY.md) — безопасность DEV-окружения
...
**⚠️ Важно:** На PROD-окружении HTTP-аутентификация НЕ создаётся автоматически.
